...
if (isset($_POST['insert'])) {
    ...
    $query = "INSERT INTO boys_users (name, reg_no, mail, dob, gender, ph_no) VALUES (?, ?, ?, ?, ?, ?)";
...
if (isset($_POST['update'])) {
    ...
    $query = "UPDATE boys_users SET name = ?, reg_no = ?, mail = ?, dob = ?, gender = ?, ph_no = ? WHERE id = ?";
    $stmt = $connection->prepare($query);
...
if (isset($_POST['delete'])) {
    ...
    // Move the deleted record to the deleted_boys table
    $query_move = "INSERT INTO deleted_records.deleted_boys (id, name, reg_no, mail, dob, gender, ph_no) SELECT id, name, reg_no, mail, dob, gender, ph_no FROM boys_users WHERE id = ?";
    $stmt_move = $connection->prepare($query_move);
    $stmt_move->bind_param("i", $id);
    if ($stmt_move->execute()) {
        // Proceed with deleting the record from the boys_users table
        $query_delete = "DELETE FROM boys_users WHERE id = ?";
        ...
        echo "<script>alert('Error moving record to deleted_boys');</script>";
    }

    $stmt_move->close();
}

// Fetch all student records or search results from boys_users table
$search_query = "";
...
$query = "SELECT * FROM boys_users $search_query";
...
<body>
    <div class="header">
        ...
        <a href="staff_home_boys.html" class="home-icon">
    ...
    <div class="container">
        <!-- Search Form -->
        <form action="student_management_boys.php" method="GET">
        ...
        <h2><a href="deleted_records_boys.php">View Deleted Records</a></h2>
    </div>
